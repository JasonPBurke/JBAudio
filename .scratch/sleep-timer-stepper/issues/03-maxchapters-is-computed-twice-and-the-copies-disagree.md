...
separate on purpose: this ticket claims _no behaviour change_ except the one
...
Triage's adopted shape (ruling 4) survived contact: `null` means _the ceiling
is not known_.
...
throwing _after_ `getActiveBookId()` succeeded drove `timer.tsx`'s catch, which
...
modal's dead branch dead _by construction_ (or comment it as unreachable and say
...
`SleepTimerOptions` is the _child_ of a `BottomSheetModal` owned by
...
`number | null`, where `null` means _"the ceiling is not known"_ — no Book, a
...
- [x] `tsc` 0, `eslint` 0 errors, full suite green.
- [x] Device pass: set an end-of-chapter timer from **both** surfaces on a
      single-file and a multi-file Book, and confirm the displayed ceiling
      matches on each.
