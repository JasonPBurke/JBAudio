...
listen's chapter — and `chapterList` consults the store _first_, so it
highlights the chapter the Book was just rewound away from.

**Status:** resolved — code complete, `tsc` 0, `eslint` clean, suite
green at 85 suites / 1050 tests. Device pass complete (2026-08-29). See
`## Answer — 2026-08-29`.
...
- `handleBookPlay.ts` contains **zero** references to `useLibraryStore`,
  ...
  race with. The comment above `:147` warns about racing _the first progress
  tick_, and that warning is about the **DB** write, which is why the DB write
...
window under test is the one _before_ any player event. Treat the
...
reads it in a way that beats a DB row into a _wrong_ result, which is the whole
...
> _This was generated by AI during ticket 01's implementation._

**2026-08-28 — filed, not fixed.** Ticket 01 explicitly forbids smuggling this
into its refactor: 01 claims _no behaviour change_ and needs a device pass that
...
fix is one line against a helper that now exists, and this is the _second_ time
...
- **The stale precondition is the normal finish, not an exotic one.** Book-end
  detection marks a Book `Finished` at _lead time_, a minute before the audio
...
**COMPLETED AND PASSED — 2026-08-29.**

Run in one session on a real device and a real Book: finished a Book by stopping
during the credits so the queue never ended, confirmed it read as Finished, then
played it again and opened the chapter list. The first chapter was highlighted,
which is the observation this fix predicts.
