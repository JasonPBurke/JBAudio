...
class MediaInfo {
    companion object {
        ...
        init {
            // Load ZenLib first (libmediainfo depends on it)
    ...
    enum class Info {
        Name, Text, Measure, Options, Name_Text, Measure_Text, Info, HowTo, Domain, Max
    }
    ...
    fun Open(fd: Int, name: String): Int = OpenFd(fd, name)

    /**
     * Convenience helper for opening a file by path from Kotlin/Java code.
     */
    fun openPath(name: String): Int = Open(name)
