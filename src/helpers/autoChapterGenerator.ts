...
/**
 * Removes auto-generated chapters from ALL books that have them,
 * reverting each to a single full-book chapter. Runs in a single transaction.
 *
 * @returns Number of books that had auto-chapters removed
 */
export async function removeAllAutoChapters(): Promise<number> {
  const booksWithAutoChapters = await database
    .get<BookModel>('books')
    .query(Q.where('has_auto_generated_chapters', true))
    .fetch();

  if (booksWithAutoChapters.length === 0) {
    return 0;
  }
  ...
  await database.write(async () => {
    ...
    for (const book of booksWithAutoChapters) {
      const existingChapters = await database
        .get<ChapterModel>('chapters')
        .query(Q.where('book_id', book.id))
        .fetch();
  ...
  return booksWithAutoChapters.length;
}
