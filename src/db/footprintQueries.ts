...
// The verdict's own projection, not a second copy of it: `chapterDuration`
// feeds its memory gate and `isAutoGenerated` its auto-chapter exclusion, so
// a narrower type here would make this file's queue-shape answer diverge from
// the queue builders' for oversized or auto-chaptered books.
type ChapterData = ShapeChapter;
...
export async function getCurrentChapterInfo(
  ...
  try {
    ...
    if (queueShapeOf(sortedChapters) === 'one-item') {
